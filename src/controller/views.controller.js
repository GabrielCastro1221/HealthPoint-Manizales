...
class ViewsController {
  ...
  renderRent = async (req, res) => {
    try {
      const rents = await rentModel
        .find({ isApproved: "approved", status: "disponible" })
        .lean();
  ...
  renderRentDetail = async (req, res) => {
    const { id } = req.params;
    try {
      const rent = await rentModel.findById(id).lean();
      if (!rent) {
        return res.render("404", { message: "Renta no encontrada" });
      }
      res.render("rentDetail", { rent });
    } catch (err) {
      res.render("404", { message: "Error al obtener los datos de la renta" });
    }
  };
}
