...
class EmailManager {
  ...
  async enviarCorreoNuevoConsultorio(consultorioData) {
    try {
      const { title, address, price, owner, owner_email, owner_phone, email } =
        consultorioData;
      const Opt = {
        from: configObject.mailer.email_from,
        to: "medicobooking21@gmail.com",
        subject:
          "Nuevo Consultorio Médico en Alquiler en Health Point Manizales",
        html: `
          <div style="font-family: Arial, sans-serif; color: #333; line-height: 1.6; max-width: 600px; margin: 0 auto; border: 1px solid #ddd; padding: 20px; border-radius: 10px;">
            <div style="text-align: center; margin-bottom: 20px;">
              <h1 style="color: #1b715e;">Health Point Manizales</h1>
            </div>
            <h2 style="color: #1b715e;">Nuevo Consultorio Médico en Alquiler</h2>
            <p>Se ha registrado un nuevo consultorio médico en nuestra plataforma con los siguientes detalles:</p>
            <ul>
              <li><strong>Título del Consultorio:</strong> ${title}</li>
              <li><strong>Dirección:</strong> ${address}</li>
              <li><strong>Precio:</strong> $${price}</li>
              <li><strong>propietario:</strong> ${owner}</li>
              <li><strong>Email del propietario:</strong> ${owner_email}</li>
            </ul>
            <p>Por favor, revise la información y tome las acciones necesarias para aprobar el perfil del consultorio.</p>
          </div>
        `,
      };
      await this.transporter.sendMail(Opt);
      logger.info(
        "Correo enviado correctamente sobre nuevo consultorio médico."
      );
    } catch (error) {
      logger.error(
        "Error al enviar correo de notificación de nuevo consultorio médico:",
        error.message
      );
    }
  }
